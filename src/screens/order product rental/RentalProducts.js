...
const RentalProducts = ({ navigation }) => {
  ...
  const fetchAllProducts = async (currentFilter) => {
    ...
    try {
      ...
      if (data.isSuccess) {
        ...
        // Lọc sản phẩm Rental: status === 1
        fullProducts = fullProducts.filter((product) => product.status === 1 && product.contractTemplateID != null);
  ...
  // Khi thay đổi pageIndex, pageSize, hoặc allProducts => update lại products
  useEffect(() => {
    updatePageProducts();
  ...
  const renderPagination = () => {
    ...
    // Nếu startPage > 1 thì hiển thị trang 1 + dấu "..."
    if (startPage > 1) {
      pages.push(
        <Button
          key="1"

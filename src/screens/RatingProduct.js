...
const RatingProduct = ({ route }) => {
    const { productID, accountID } = route.params;
    ...
    const handleSubmit = async () => {
        if (!productID || !accountID) {
          Alert.alert('Lỗi', 'Thông tin sản phẩm hoặc tài khoản không hợp lệ.');
          return;
        }
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(`HTTP ${response.status}: ${data.message || 'Bad Request'}`);
          }

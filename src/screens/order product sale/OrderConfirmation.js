...
const OrderConfirmation = ({ route, navigation }) => { // Đảm bảo navigation được truyền vào
      ...
      if (response.ok && data?.result) {
        ...
        Alert.alert(
          ...
          [
            {
              text: 'OK',
              onPress: () => navigation.navigate('SaleProduct'), 
            },
          ]
        );
        
        if (data?.result) {
          Linking.openURL(data.result).catch((err) =>
        ...
        console.error('Lỗi khi tạo đơn hàng(1):', data.messages || 'Lỗi không xác định.');
        Alert.alert('Lỗi', 'Không thể tạo đơn hàng.');
      }
    } catch (error) {
      console.error('Lỗi khi gửi đơn hàng(2):', error);
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Thanh toán" onPress={handleCompleteOrder} />

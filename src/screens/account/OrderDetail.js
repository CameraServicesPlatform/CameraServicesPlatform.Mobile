...
const OrderDetail = ({ route }) => {
  ...
  const orderStatusReturnMapping = {
    ...
    3: { label: "Đã nhận sản phẩm", color: "#17a2b8" },
  ...
  const handlePickup = async () => {
    try {
      ...
      if (response.ok) {
        Alert.alert('Thành công', 'Đã xác nhận rằng bạn đã nhận sản phẩm từ cửa hàng.');
        reloadOrderDetails();
      } else {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        ...
        {orderType === 1 &&  orderStatus === 1 &&(
          <TouchableOpacity style={[styles.button, styles.buttonPrimary]} onPress={handlePickup}>
            <Text style={styles.buttonText}>Xác nhận đã nhận sản phẩm thuê</Text>
          </TouchableOpacity>
        )}
